...
library(reshape2)
library(lubridate)
basin_results_df <- basin_results_df0
...
#offset["AirT"] <- 0
multp["Pnet.mm."] <- -(area/(dt*1000)/10)^(-1)
multp["Evap_surface.mm."] <- (area/(dt*1000)/10)^(-1)
multp["Transpiration_canopy.mm."] <- (area/(dt*1000)/10)^(-1)
offset["Pnet.mm."] <- ost <- 4.5
multp["Qtot.m3.s."] <- 1
...
itref <- "Pnet.mm."
itref <- "Qtot.m3.s."
itnoref <- "Pnet.mm."###cols_[cols_!=itref][1]
...
gp <- gp+ geom_line(aes(color = variable), 
 ...
 ##  scale_linetype_manual(values= c(1,0.5,0.5))+
  ...
  scale_y_continuous(
    "Discharge [m3/s]", 
    sec.axis = sec_axis(~ . * a+b, name = "Precipitation [mm]",breaks=breaks,labels=labels)
  )
gp
```
