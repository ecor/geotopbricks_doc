...
## Aggregate maps of seasonal maximim snow depth
cm <- system.time(outseas <- out$file %>% str_split(pattern=",") %>% lapply(FUN=stack) %>% lapply(FUN=function(x,...){max(x,na.rm=TRUE)}) %>% stack() %>% stack(alt_map_z))
...
file_oo2_csv <- file_oo2 %>% paste0("max_altitude.csv")
