...
## Aggregate maps of seasonal mean snow depth (sum)

cm <- system.time(outseas <- out$file %>% str_split(pattern=",") %>% lapply(FUN=stack) %>% lapply(FUN=function(x){sum(x)}) %>% stack() %>% stack(alt_map_z))
...
file_oo2_csv <- file_oo2 %>% paste0("nday_altitude.csv")
